---
title: "Best States for Black, Hispanic, Asian, and Female Tech -- Version 0.99"
...
Answer 3C ... The fourth column of Table 3ABC (below) shows the percentage of the American techs in the second column who were female. Females were an overall minority of `r dfTable3ABC[dfTable3ABC$Occupation=="ALL","F15_%"] `% of all professionals in tech, and were a minority in every tech category, ranging from a low value of `r prettyNum(min(dfTable3ABC[, "F15_%"]), scientific=FALSE)`% for to a high value of `r prettyNum(max(dfTable3ABC[, "F15_%"]), scientific=FALSE)`%.

<b>Table 3ABC -- Profile of American Professionals in U.S.Tech in 2015</b>   
```{r, echo=FALSE, cache=FALSE,  comment=NA}
  colnames(dfTable3ABC) <- c("Occupation", "Tech15", "TS_%", "Male", "M_%", "Female", "F15_%")
...
Answer 3E ... As per Table 3E (below), there were xxx tech professionals in the U.S. of which xxx were female, i.e. xxx. There were xxx professionals from Asia and xxx from coutries outside of Asia. . The largest occupational category in each case was software devlopers, with xxx from Asia and xxx from outside Asia blah blah ...

<b>Table 3E -- Profile of Foreign Professionals Working in U.S. Tech in 2015</b>   
```{r, echo=FALSE, cache=FALSE,  comment=NA}
colnames(dfTable3E) <- c("Occupation", "Foreign", "As2015", "AS_%", "NAs2015", "NAS_%", "F15_%")
...
Answer 3F ... As per Table 3F (below), the number of foreign tech professionals from Asia who were working in the U.S. increased by xxx percent, from xxx in 2010 to xxx in 2015. Software developers from Asia increased by xxx percent from xxx to xxx.
...
Answer 3G ... As per Table 3H (below), the number of foreign tech professionals who came from countries outside of Asia increased by xxx percent, from xxx in 2010 to xxx in 2015. Software developers who came from countries outside of Asia increased by xxx percent, from xxx to xxx  

<b>Table G -- Profile of Changes in Foreign Professionals Not from Asia Between 2010 and 2015</b>   
```{r, echo=FALSE, cache=FALSE,  comment=NA}
...
* Map 4A displays light hues for all states, indicating that White American techs are spread across a large number of states, with Californiand Texas having marginally higher percentages than the other states. 

* Map 4B shows that most Black American techs are located in the Eastern and Southern states, with the largest number working in Georgia, second in Texas, and third in Virginia
...
* Map 4D shows that Hispanic American techs are concentrated in California, Texas, and Florida   

* Map 4E shows that Asian American female techs are highly concentrated in California.

* Map 4F shows that non-Asian American female techs have their largest shares of tech in California, Texas, and Florida, but the distribution is far more dispersed than the distribution for Asian female techs.   
...
3. <b>SS_%</b> -- The state's share of each group's national total. More specifically, the percent of all American techs in a group that worked in each state. For example, Table 4A shows that  `r dfTable4A["California", "perState"]`  percent of all White techs worked in California in 2015, and `r dfTable4A["Texas", "perState"]`  percent worked in Texas.<br /> <br />
...
8. <b>Par</b> -- Parity = The ratio of the  percent of the state's tech population that belonged to the table's group divided by the percent of the state's total population that belonged to that group, i.e., T_% / P_%. For example, Table 4A shows that the White parity ratio for California was  `r dfTable4A["California", "Parity"]`, which is `r dfTable4A["California", "perWhiteTech"]`% divided by `r dfTable4A["California", "perWhitePop"]`%<br /><br />
...
* Despite California's relatively low Hispanic American parity, the fifth column (T_%) of Table 4D shows that `r dfTable4D["California", "T_%"]` percent of the techs who worked in California were Hispanic. This figure is remarkable because it is two to three times as high as the 2 percent to 4 percent employment rates for Hispanic techs that have been reported by Silicon Valley's biggest firms in the last three years. It suggests that willingness to hire Hispanic techs was low throughout the state, but substantantially lower in the Valley.    
 ...
 * By comparing the California row of the third columns (SS_%) of Tables 4C and 4E, the reader will see that that Asian Americanfemale tech professionals were as highly concentrated in California as the entire male and female Asian American cohort. To be specific, `r dfTable4E["California", "SS_%"]` percent of all Asian American female techs in the U.S. were in California (Table 4E), which is essentially the same as the `r dfTable4C["California", "SS_%"]` percent of all Asian American tech professionals who were in California (Table 4C).
 
 * Non-Asian American females showed no such concentration in California or in any other state. To be sure, California employed the most non-Asian female techs (`r dfTable4F["California", "SS_%"]` %), but this was not much larger than the percentage in Texas (`r dfTable4F["Texas", "SS_%"]` %), which was not much larger than in New York (`r dfTable4F["New York", "SS_%"]` %), etc. 
...
```{r, echo=FALSE, cache=TRUE, comment=NA}
    ### Change of column names is permanent from here on ... matches displayed colum names
    colnames(dfTable4H) <- c("State", "Foreign", "SS_%", "NAsiaTech", "T_%")
print(format(dfTable4H[1:11,], big.mark = ","), row.names=FALSE)  

```
<b>Comments about Table 4G and 4H -- Foreign Tech Professionals from Asia </b>    

* As shown in the SS_% column in both tables, foreign tech professionals had high concentrations in California, with California employing xxx percent of the xxxx foreign techs from Asia and xxx percent of the xxx foreign techs from outside Asia. Texas ranked second for both groups. Its xxx percent and xxx percent concentrations were substantially lower than foreign concentrations in California for techs from Asia and outside Asia, but was only marginally higher than the xxx percent and xxx percent for New York, the third ranking state for both groups.   

* The importance of California in the U.S tech sector requires closer scrutiny of its employment of foreign professionals. The xxx professionals in California's cohort from Asia was xxx times as large as the xxx Black American techs (Table 4B) who worked in California, and xxx times as large as the xxx Hispanic American techs (Table 4D) who worked in California.   

* The xxx foreign profrofessionals in California from countries outside of Asia was xxx times as large as California's xxx Black American techs (Table 4B) and xxx times as large as its xxx Hispanic American techs (Table 4D).
