---
title: "Best States for American Black, Hispanic, Asian, and Female Tech -- Version 0.99F"
...
```{r LOAD, echo=FALSE, cache=FALSE}
    ...
    load("dfCensus2.rda") 
    load("dfTab1A1B2A2B3ABCDEFG.rda")
    load("dfTab4.rda")   
    load("dfMap4.rda")
...
* In 2015 there were approximately XXXXX professionals in the entire U.S. information technology sector. Of these approximately  `r prettyNum(dfTable1B["Num","ALL"], big.mark=",", scientific=FALSE)` were Americans; xxx were foreign professionals  (See Table XXX)

* In 2015 there were approximately xxx White Americans in U.S. tech; `r prettyNum(dfTable1B["Num","Black"], big.mark=",", scientific=FALSE)` Black Americans; `r prettyNum(dfTable1B["Num","Hispanic"], big.mark=",", scientific=FALSE)` Hispanic Americans; `r prettyNum(dfTable1B["Num","Asian"], big.mark=",", scientific=FALSE)` Asian Americans; and `r prettyNum(dfTable2B["Num","Female"], big.mark=",", scientific=FALSE)` female Americans. (See Table 1B and Table 2B.)   

* Approximately xxxx foreign employees in the U.S. tech sector were from Asia; and xxxx were from non-Asian countries. (See Table XXX)
...
* The subgroup of Asian American females occupied disproportionately larger shares of the tech sectors in most states. Indeed, their tech shares ranged as high as `r dfTable6["FemAsian", "Max"]` times their shares of state workforce populations. (See Table 4E and Table 6.) By contrast, the subgroup of non-Asian American females occupied much smaller shares of state tech sectors than their shares of state workforce populations. (See Tables 2B, 3 and 6.)   

* The total U.S. tech sector grew by xxx percent between 2010 and 2015, i.e., from xxx to xxx. The number of tech jobs held by American techs grew by xxx percent during those years, i.e., xxx to xxx; the number held by White Americans grew by xxx percent, i.e., from xxx to xxx; tech jobs held by Black Americans grew by xxx percent, i.e., from xxx to xxx; tech jobs held by Asian Americans grew by xxx percent, i.e., from xxx to xxx; and  tech jobs held by Hispanic Americans grew by xxx percent, i.e., from xxx to xxx. (See tables 3, 3AA, 3BB, 3CC, and 3DD)  

* The number of tech jobs held by American females grew by xxx percent between 2010 and 2015, i.e., from xxx, to yyy. However American females occupied a smaller percentage of the tech jobs held by American tech professionals in 2015 than they did in 2010, i.e.,  `r prettyNum(dfTable3D[1,"perF10"], big.mark=",", scientific=FALSE)` xxx percent in 2010 compared to only  XXX percent in 2015. (See Table 3ABC and 3D)

* The number of tech jobs held by foreign professionals in U.S. tech grew by xxx percent between 2010 and 2015, i.e., from xxx to xxx. The nmber of foreign techs from Asia grew by xx percent, i.e., from xxx to xxx; the number from countries outside Asia grew by xxx percent, i.e., from xxx to xxx

* The increase in foreign tech professionals was highest in California. The number of foreign tech professionals working in California increased by xxx percent between 2010 and 2015, i.e. from xxx  to xxx. In other words, California accounted for xxx percent of the xxx increase in the number of foreign professionals working in U.S. tech during that time frame. The remaining xxx percent of the increase was spread across the other 49 states and the District of Columbia. (See Table XXX)

* The number of foreign professionals imported from Asia in 2015 was substantially larger than the `r prettyNum(dfTable1D["Num","Black"], big.mark=",", scientific=FALSE)` Black American or the  `r prettyNum(dfTable1D["Num","Hispanic"], big.mark=",", scientific=FALSE)` Hispanic American professionals employed by U.S. tech in 2015. Indeed,  the `r prettyNum(dfTable1E[1,"Foreign"], big.mark=",", scientific=FALSE)` xxxx combined total of foreign professionals imported from Asia and from elsewhere in 2015 was almost as large as the  `r prettyNum((as.numeric(gsub(",", "",dfTable1D["Num","Black"])) + as.numeric(gsub(",", "", dfTable1D["Num","Hispanic"]))), big.mark=",", scientific=FALSE)` combined total of Black and Hispanic American professionals employed by the U.S. tech sector.  (See Tables 3D, 3E, and 3F.)
...
<i>Answer 1A</i> ... blah blah See Table 1A
...
<i>Answer 1C</i> ... Most of the tables in this report will deal with American techs or foreign techs, but not both. Therefore the totals in the second and third columns of this small table will recur in many subsequent tables.  
...
<i>Answer 1E</i> ...  blah blah Table Table 1E  
...
<i>Question 3F</i> -- How many foreign male vs. female tech were in each category in 2010 and 2015 in Texas? 

 <i>Answer 3F</i> ... blah, blah, blah
 
 <b>Table 3F -- Texan Foreign Male/Female Composition of U.S. Tech in 2015</b>  
```{r, echo=FALSE, cache=FALSE, comment=NA}
   print(format(dfTable3F, big.mark=","), row.names=FALSE, quote=FALSE)
```

<b>Table 3FF -- Changes in the Texas Foreign Male/Female Composition of U.S. Tech between 2010 and 2015</b> 
```{r, echo=FALSE, cache=FALSE, comment=NA}
   print(format(dfTable3FF, big.mark=","), row.names=FALSE, quote=FALSE)
```
<small><i><center><a href="https://archimedes2kxx.github.io/BestStatesForBlackTech/Report-3.html" target="_self">back-to-top</a></center></i></small>    

<i>Question 3G</i> -- How many foreign male vs. female tech were in each category in 2010 and 2015 in California? 

 <i>Answer 3F</i> ... blah, blah, blah
 
 <b>Table 3G -- California's Foreign Male/Female Composition of U.S. Tech in 2015</b>  
```{r, echo=FALSE, cache=FALSE, comment=NA}
   print(format(dfTable3G, big.mark=","), row.names=FALSE, quote=FALSE)
```

<b>Table 3GG -- Changes in the California Foreign Male/Female Composition of U.S. Tech between 2010 and 2015</b> 
```{r, echo=FALSE, cache=FALSE, comment=NA}
...
* Table 5 showed that the national parity rating for Black Americans in tech was `r dfTable5["Black", "Par"]`, i.e., `r dfTable1D["Per","Black"]` divided by `r dfTable1B["Per","Black"]`, which was more than half, i.e., greater than 0.50. The vast majority of tech jobs required bachelors degrees. This meant that, contrary to popular misconceptions, <b>the nation's Black academic pipelines were more than half-full; they were not perilously close to empty</b>.  
