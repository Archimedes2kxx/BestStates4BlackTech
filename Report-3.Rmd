---
title: "Best States for American Black, Hispanic, Asian, and Female Tech -- Version 0.99"
...
* In 2015 there were approximately `r prettyNum(dfTable1B["Num","ALL"], big.mark=",", scientific=FALSE)` American employees in the entire U.S. information technology sector ==>  `r prettyNum(dfTable1B["Num","Black"], big.mark=",", scientific=FALSE)` were Black Americans; `r prettyNum(dfTable1B["Num","Hispanic"], big.mark=",", scientific=FALSE)` were Hispanic Americans; `r prettyNum(dfTable1B["Num","Asian"], big.mark=",", scientific=FALSE)` were Asian Americans; and `r prettyNum(dfTable2B["Num","Female"], big.mark=",", scientific=FALSE)` were female Americans. (See Table 1B and Table 2B.)      
...
* If all other things were equal, one would expect that the percentage of a state's tech sector that was Black would be about the same as the percentage of the state's total population that was Black. One would expect similar parity for the percentages of Hispanics and for females. But all things were not equal, so the percentage of the tech sector of most states that was Black, Hispanic, or female Americans was substantially smaller than the percentage of the state's total population that was Black, Hispanic, or female. By contrast, the percentage of state tech sectors that was Asian American was usually greater than their population percentage and ranged as high as `r dfTable6["Asian", "Max"]` times the Asian population percentage. (See Tables 4C and 6)  

* Female American tech professionals only held `r prettyNum(dfTable2B["Per","Female"])` percent of the nation's `r prettyNum(dfTable2B["Num","ALL"], big.mark=",", scientific=FALSE)` of the tech positions that were occupied by Americans. But Asian American females occupied disproportionately larger shares of the tech sectors in most states than their share of each state's population. Indeed, their tech shares ranged as high as `r dfTable6["FemAsian", "Max"]` times their shares of state populations. (See Table 4E and Table 6.) By contrast, non-Asian American females occupied much smaller shares of the state tech sectors than their shares of the state populations. (See Tables 2B, 3 and 6.) Furthermore American females held a smaller share of jobs allocated to American tech professionals in 2015 than they did in 2010, i.e., `r prettyNum(dfTable3ABC[1,"perF15"], big.mark=",", scientific=FALSE)` percent in 2015 vs. `r prettyNum(dfTable3D[1,"perF10"], big.mark=",", scientific=FALSE)` percent in 2010. (See Table 3ABC and 3D)

* Finally, in 2015 the U.S. information technology sector employed `r prettyNum(dfTable3F[1,"As2015"], big.mark=",", scientific=FALSE)` foreign  professionals from Asia who moved here to work for their clients in this country. U.S. tech also imported `r prettyNum(dfTable3G[1,"NAs2015"], big.mark=",", scientific=FALSE)` foreign professionals from countries outside of Asia. It should be noted that the number of  professionals from Asia in 2015 was `r prettyNum(dfTable3F[1,"perCh"], big.mark=",", scientific=FALSE)` percent higher than its `r prettyNum(dfTable3F[1,"As2010"], big.mark=",", scientific=FALSE)` level in 2010. The number of professionals imported from Asia was also substantially larger than the `r prettyNum(dfTable1B["Num","Black"], big.mark=",", scientific=FALSE)` Black American or the  `r prettyNum(dfTable1B["Num","Hispanic"], big.mark=",", scientific=FALSE)` Hispanic American professionals employed by U.S. tech in 2015. Indeed,  the `r prettyNum(dfTable3E[1,"Foreign"], big.mark=",", scientific=FALSE)` combined total of foreign professionals imported from Asia and from elsewhere who were working in the U.S. in 2015 was almost as large as the  `r prettyNum((as.numeric(gsub(",", "",dfTable1B["Num","Black"])) + as.numeric(gsub(",", "", dfTable1B["Num","Hispanic"]))), big.mark=",", scientific=FALSE)` combined total of Black and Hispanic American professionals.  (See Tables 3D, 3E, and 3F.)
...
#### Question 1A -- What was the White, Black, Asian, and Hispanic population of the U.S. in 2015?    
Answer 1A ... In 2015, as shown in Table 1A (below), the largest of the four racial/ethnic groups discussed in this report was White (`r prettyNum(dfTable1A["Num","White"], big.mark=",", scientific=FALSE)`); the smallest was Asian (`r prettyNum(dfTable1A["Num","Asian"], big.mark=",", scientific=FALSE)`). Hispanics, the largest minority (`r prettyNum(dfTable1A["Num","Hispanic"], big.mark=",", scientific=FALSE)`), had grown substantially larger than Blacks (`r prettyNum(dfTable1A["Num","Black"], big.mark=",", scientific=FALSE)`).
...
Answer 1B ... As shown in Table 1B (below), the U.S. Census Bureau estimated that there were `r prettyNum(dfTable1B["Num","ALL"], big.mark=",", scientific=FALSE)` American tech professionals in the U.S. in 2015.  White Americans were a substantial majority (`r prettyNum(dfTable1B["Num","White"], big.mark=",", scientific=FALSE)`) of the tech sector (`r prettyNum(dfTable1B["Per","White"], big.mark=",", scientific=FALSE)`%); Asians placed second (`r prettyNum(dfTable1B["Num","Asian"], big.mark=",", scientific=FALSE)`); Blacks showed third (`r prettyNum(dfTable1B["Num","Black"], big.mark=",", scientific=FALSE)`); and Hispanic Americans came in fourth  (`r prettyNum(dfTable1B["Num","Hispanic"], big.mark=",", scientific=FALSE)`)
...
<b>Table 2A -- Male/Female Composition of the U.S. population in 2015</b>   
```{r, echo=FALSE, cache=FALSE, comment=NA}
  print(format(dfTable2A, big.mark=","), row.names=TRUE)
```

#### Question 2B -- What was the ratio of American males to American females in the tech sector in 2015? 
...
<i>Note: The five tables that provide the answers to the third set of questions display profiles of the various groups considered in this report. The first two tables, Table 3ABC and Table 3D provide profiles of American professionals in the U.S. tech sector; the last three tables, Table 3E, 3F, and 3G provide profiles of foreign professionals in the U.S. tech sector. </i>  

Answer 3A ... For the purposes of this report, "information technology" was defined as the thirteen standard occupation categories (SOC) that appear in the first column of Table 3ABC (below). Twelve of these categories related to software, e.g., Software Developers, Computer Programmers, and Database Administrators; only one relates to hardware, i.e., Computer Hardware Engineers.  

#### Question 3B -- How many American techs were in each category?        
Answer 3B ... The total number of American techs in each category appear the second column of Table 3ABC (below). The top row shows that there were approximately `r prettyNum(dfTable3ABC[dfTable3ABC$Occupation=="ALL","Tech15"], big.mark=",", scientific=FALSE)` professionals in  all tech categories. 

#### Question 3C -- What was the American male, American female breakdown of employment within these categories?         
Answer 3C ... The fourth column of Table 3ABC (below) shows the percentage of the American techs in the second column who were female. Females were an overall minority of `r dfTable3ABC[dfTable3ABC$Occupation=="ALL","perF15"] `% of all professionals in tech, and were a minority in every tech category, ranging from a low value of `r prettyNum(min(dfTable3ABC[, "perF15"]), scientific=FALSE)`% for to a high value of `r prettyNum(max(dfTable3ABC[, "perF15"]), scientific=FALSE)`%.

<b>Table 3ABC -- Profile of American Professionals in U.S.Tech in 2015</b>   
```{r, echo=FALSE, cache=FALSE,  comment=NA}
  colnames(dfTable3ABC) <- c("Occupation", "Tech15", "TS_%", "Male", "M_%", "Female", "perF15")
   print(format(dfTable3ABC[,c(1:3, 6, 7)], big.mark=","),row.names=FALSE) 
```

#### Question 3D -- How many American professionals were added to U.S. tech between 2010 and 2015?    

Answer 3D ... As per first row of Table 3D (below), Americans held  `r prettyNum(dfTable3D[dfTable3D$Occupation=="ALL","Tech10"], big.mark=",", scientific=FALSE)` jobs in tech in 2010 and  `r prettyNum(dfTable3D[dfTable3ABC$Occupation=="ALL","Tech15"], big.mark=",", scientific=FALSE)` tech jobs in 2015. This represented a  `r prettyNum(dfTable3D[dfTable3ABC$Occupation=="ALL","perCh"], scientific=FALSE)` percent increase of  `r prettyNum(dfTable3D[dfTable3ABC$Occupation=="ALL","Change"], big.mark=",", scientific=FALSE)` jobs. The second row shows that the largest category, software developers, increased from `r prettyNum(dfTable3D[dfTable3D$Occupation=="SOFTWARE DEVELOPERS","Tech10"], big.mark=",", scientific=FALSE)` jobs in 2010 to `r prettyNum(dfTable3D[dfTable3D$Occupation=="SOFTWARE DEVELOPERS","Tech15"], big.mark=",", scientific=FALSE)` jobs in 2015, thereby achieving a `r prettyNum(dfTable3D[dfTable3D$Occupation=="SOFTWARE DEVELOPERS","perCh"], big.mark=",", scientific=FALSE)` percent increase of  `r prettyNum(dfTable3D[dfTable3D$Occupation=="SOFTWARE DEVELOPERS","Change"], big.mark=",", scientific=FALSE)` jobs

<b>Table 3D -- Profile of American Professionals Added to U.S.Tech Between 2010 and 2015</b>   
```{r, echo=FALSE, cache=FALSE,  comment=NA}
   print(format(dfTable3D, big.mark=","),row.names=FALSE) 
```
<small><i><center><a href="https://archimedes2kxx.github.io/BestStatesForBlackTech/Report-3.html" target="_self">back-to-top</a></center></i></small>    

#### Question 3E -- How many foreign tech professionals worked in the U.S. in 2015?   

Answer 3E ... As per the first row of Table 3E (below), there were `r prettyNum(dfTable3E[dfTable3E$Occupation=="ALL","Foreign"], big.mark=",", scientific=FALSE)` tech professionals in the U.S. in 2015 of which `r prettyNum(dfTable3E[dfTable3E$Occupation=="ALL","perF15"], big.mark=",", scientific=FALSE)`  percent were female. There were `r prettyNum(dfTable3E[dfTable3E$Occupation=="ALL","Asia"], big.mark=",", scientific=FALSE)` professionals from Asia and `r prettyNum(dfTable3E[dfTable3E$Occupation=="ALL","NAsia"], big.mark=",", scientific=FALSE)` from coutries outside of Asia. . The largest occupational category in each case was software devlopers, with `r prettyNum(dfTable3E[dfTable3E$Occupation=="SOFTWARE DEVELOPERS","Asia"], big.mark=",", scientific=FALSE)` from Asia and `r prettyNum(dfTable3E[dfTable3E$Occupation=="SOFTWARE DEVELOPERS","NAsia"], big.mark=",", scientific=FALSE)` from outside Asia.

<b>Table 3E -- Profile of Foreign Professionals Working in U.S. Tech in 2015</b>   
```{r, echo=FALSE, cache=FALSE,  comment=NA}
colnames(dfTable3E) <- c("Occupation", "Foreign", "As2015", "perAs", "NAs2015", "perNAs", "perF15")
   print(format(dfTable3E, big.mark=","),row.names=FALSE) 
```


#### Question 3F -- How many foreign professionals from Asia were added to U.S. tech between 2010 and 2015?   
Answer 3F ... As per the first row of Table 3F (below), the total number of foreign tech professionals from Asia who were working in the U.S. increased by `r prettyNum(dfTable3F[dfTable3F$Occupation=="ALL","perCh"], big.mark=",", scientific=FALSE)` percent, from `r prettyNum(dfTable3F[dfTable3F$Occupation=="ALL","As2010"], big.mark=",", scientific=FALSE)` in 2010 to  `r prettyNum(dfTable3F[dfTable3F$Occupation=="ALL","As2015"], big.mark=",", scientific=FALSE)` in 2015. Software developers from Asia increased by `r prettyNum(dfTable3F[dfTable3F$Occupation=="SOFTWARE DEVELOPERS","perCh"], big.mark=",", scientific=FALSE)` percent from `r prettyNum(dfTable3F[dfTable3F$Occupation=="SOFTWARE DEVELOPERS","As2010"], big.mark=",", scientific=FALSE)` to `r prettyNum(dfTable3F[dfTable3E$Occupation=="SOFTWARE DEVELOPERS","As2015"], big.mark=",", scientific=FALSE)`

<b>Table 3F -- Profile of Changes in Foreign Professionals from Asia Between 2010 and 2015</b> 
```{r, echo=FALSE, cache=FALSE,  comment=NA}
   print(format(dfTable3F, big.mark=","),row.names=FALSE) 
```

#### Question 3G -- How many foreign professionals not from Asia were added to U.S. tech between 2010 and 2015?   
Answer 3G ... As per the first row of Table 3H (below), the total number of foreign tech professionals who came from countries outside of Asia increased by `r prettyNum(dfTable3G[dfTable3G$Occupation=="ALL","perCh"], big.mark=",", scientific=FALSE)` percent, from `r prettyNum(dfTable3G[dfTable3G$Occupation=="ALL","NAs2010"], big.mark=",", scientific=FALSE)` in 2010 to `r prettyNum(dfTable3G[dfTable3G$Occupation=="ALL","NAs2015"], big.mark=",", scientific=FALSE)` in 2015. Software developers who came from countries outside of Asia increased by `r prettyNum(dfTable3G[dfTable3G$Occupation=="SOFTWARE DEVELOPERS","perCh"], big.mark=",", scientific=FALSE)` percent, from `r prettyNum(dfTable3G[dfTable3G$Occupation=="SOFTWARE DEVELOPERS","NAs2010"], big.mark=",", scientific=FALSE)` to `r prettyNum(dfTable3G[dfTable3G$Occupation=="SOFTWARE DEVELOPERS","NAs2015"], big.mark=",", scientific=FALSE)`  

<b>Table 3G -- Profile of Changes in Foreign Professionals Not from Asia Between 2010 and 2015</b>   
```{r, echo=FALSE, cache=FALSE,  comment=NA}
...
* Map 4A displays light hues for all states, indicating that White American techs were spread across a large number of states, with Californiand Texas having marginally higher percentages than the other states. 
...
7. <b>perP</b> -- Percent of the state's total population that belonged to the table's  group. For example, Table 4A shows that in 2015, `r dfTable4A["California", "perWhitePop"]` percent of the total population of California was White, and `r dfTable4A["Texas", "perWhitePop"]` percent of the total population of Texas was white<br />.<br />
8. <b>Par</b> -- Parity = The ratio of the  percent of the state's tech population that belonged to the table's group divided by the percent of the state's total population that belonged to that group, i.e., perT / perP. For example, Table 4A shows that the White parity ratio for California was  `r dfTable4A["California", "Parity"]`, which is `r dfTable4A["California", "perWhiteTech"]`% divided by `r dfTable4A["California", "perWhitePop"]`%<br /><br />
...
<b>Plots 5 -- Plots of American Tech Employment vs Race/Ethnic/Female Group Population</b>   
...
* As per the values of the beta1000 slopes, If each ethnic/racial group's population increased by 1,000, Asian techs would increase by `r beta1000["Asian"]`; White Americans would increase by `r beta1000["White"]`, Black Americans would increase by `r beta1000["Black"]`; and Hispanic Americans would increase by `r beta1000["Hispanic"]`. The Asian Americann line has the steepest slope; the Hispanic American line has the flattest slope. Asian American female techs had the second steepest slope, beta1000 = `r beta1000["FemAsian"]`; non-Asian American female techs had the second flattest slope, beta1000 = `r beta1000["FemNonAsian"]`.
...
Table 1A showed that `r dfTable1A["Per","Black"]` percent of the U.S. population in 2015 was Black American; and Table 1B showed that the nation's `r dfTable1B["Num","Black"]`   Black American tech professionals represented `r dfTable1B["Per","Black"]` percent of the nation's `r dfTable1B["Num","ALL"]` American tech professionals. 

* Table 5 showed that the national parity rating for Black Americans in tech was `r dfTable5["Black", "Par"]`, i.e., `r dfTable1B["Per","Black"]` divided by `r dfTable1A["Per","Black"]`, which was more than half, i.e., greater than 0.50. The vast majority of tech jobs required bachelors degrees. This meant that, contrary to popular misconceptions, <b>the nation's Black academic pipelines were more than half-full; they were not perilously close to empty</b>.  
...
I used the DataFerret to download a subset of the PUMS 2015 sample about the sex, state of workplace, citizenship, continent on which they were born, race, and Hispanic status of employees in the tech sector in all 50 states plus the District of Columbia:<br />   
...
The scripts and embedded functions should be run in numerical order ==> Data-1A.R and Data-1B.R create the data frames required by Stats-2A.R, Stats-2B.R, and Stats-2C.R. The Stats files create the tables, maps, graphs, and regressions required by the R-markdown script in the Report-3.Rmd file that generates the report.
