...
file = "PEP_2015_PEPSR6H_with_ann.csv"
...
dfNonHispanics <- subset(dfStatesPop1, Year.id=="est72015" & Sex.id =="totsex"  & Hisp.id=="nhisp", select=c(wa:tom)) ### wa = white americans ... tom = two or more groups
head(dfNonHispanics)

### 3. Get data for hispanics
dfHispanics <- subset(dfStatesPop1, Year.id=="est72015" & Sex.id =="totsex"  & Hisp.id=="hisp", select=totpop)
...
dfTotPop <- subset(dfStatesPop1, Year.id=="est72015" & Sex.id=="totsex" & Hisp.id=="tothisp", select=c(`GEO.display-label`, totpop)) ### get states and total population
head(dfTotPop)

### 4F. Get total female pop of the states
dfFemale <- subset(dfStatesPop1, Year.id=="est72015" & Sex.id=="female" & Hisp.id=="tothisp", select=c(`GEO.display-label`, totpop))
colnames(dfFemale) <- c("State", "Female")
head(dfFemale) ### correct total U.S.

### 4AF. Get data for all Asian females
dfFemAsian <- subset(dfStatesPop1, Year.id=="est72015" & Sex.id =="female"  & Hisp.id=="nhisp", select=c(`GEO.display-label`,aa)) ### Asian American females
colnames(dfFemAsian) <- c("State", "FemAsian")
head(dfFemAsian) ### Correct total U.S. Asian female

##############################
### Modify code to add Female, femAsian, femNonAsian, per_female, per_femAsian, per_femNonAsian

dfTotPopFemale <- dfFemale 
dfTotPopFemale$FemAsian <- dfFemAsian$FemAsian
head(dfTotPopFemale)
...
dfStatesPop2$FemNonAsian <- dfStatesPop2$Female - dfStatesPop2$FemAsian
