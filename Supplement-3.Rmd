---
title: "Foreign Professionals: U.S. Tech's Other Pipelines -- Version 0.78"
...
* The percentage of the foreign female software developers declined from 21.1xxx % in 2010 down to 19.5xxx % in 2015 (Tables 3E and 3EE). The female component of American software developers also declined, from 22.0xxx % in 2010 down to 20.3xxx % in 2015 (Tables 3Z and 3ZZ).

* The percentage of the foreign female database administrators declined from 39.8xxx % in 2010 down to 23.8xxx % in 2015 (Tables 3E and 3EE). The female component of American database administrators also declined, from 39.8xxx % in 2010 down to 37.1xxx % in 2015 (Tables 3Z and 3ZZ).   

* The percentage of the foreign female computer/research scientists declined from 26.5xxx % in 2010 down to 10.3xxx % in 2015 (Tables 3E and 3EE). The female component of the American computer/research scientists also declined, from 39.8xxx % in 2010 down to 29.9xxx % in 2015 (Tables 3Z and 3ZZ).  
...
U.S. Census data that shows the geographic distribution of foreign tech professionals is presented in two formats: first, as a set of choropleth maps and second, as a corresponding set of tables. The maps are shown first because it’s easier to see the workplace patterns by looking at multicolored maps than by scanning columns of data in long tables. Indeed, a quick scan of the maps makes it easier to spot the same patterns in the data tables. Each map colors the states with the largest number of tech workers in the brightest colors, and colors the states with the least tech workers in the faintest colors. The same colors represent the same percentages on both maps.
...
* California is brightly colored on both maps, signifying the highest concentrations of Asian and non-Asian techs in that state. Texas is the second brightest state, signifying the next highest concentration of foreign techs.  New York is third.

* Overall both maps assign the same colors to all of the other states, with one notable exception. Florida is dull gray on Map 4G (Asian techs), but is almost as bright as New York on Map 4H (non-Asian techs). This signifies that Florida has about the same percentage of foreign techs from non-Asian countries as New York; but is way down in the pack as a work state for Asian techs. Given the large numbers of foreign professionals from Latin American countries in Florida, its prominence as a work state for non-Asian foreign techs is not surprising.
...
2. <b>AllForeign</b> – The total number of all foreign techs in the U.S. (Asian + non-Asian) who worked in each state. The data in this column is repeated in both tables, but the order of the rows differs from one table to the next. For example, Table 4G shows that the total number of foreign techs who worked California in 2015 was xxx513,031. Table 4G shows that total number of foreign techs in Florida was xxx328,635 

3. <b>perSS</b> – The state’s share of all Asian or non-Asian techs in the U.S. For example, Table 4G shows that 9.3 percent of the techs from Asian countries worked in California; and xxx7 percent of all techs from Asian countries worked in Texas.

4. <b>Asian/Non-Asian</b>  The data in this column shows the number of Asian or non-Asian techs in each state. For example, Table 4H shows that xxx techs from Asian countries worked in California, and xxx205,636 techs from Asian countries worked in Texas. The rows of each table are ordered by the data in this column, i.e., by the number of techs in each state. States with higher numbers are at the top of the table; states with lower numbers are at the bottom.

5. <b>perT</b> – Percent of all foreign techs in each state who were from Asian countries (Table 4G) or were from non-Asian countries (Table 4H). For example, Table 4G shows that xxx52.6 percent of all techs in the U.S. who were from from Asian countries worked in California. Table 4H shows that  xxx62.6 percent of all the techs from non-Asian countries worked in California.
